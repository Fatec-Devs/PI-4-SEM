const { PrismaClient } = require('@prisma/client');
const { PrismaPg } = require('@prisma/adapter-pg');
const pg = require('pg');

// Criar pool do PostgreSQL
const pool = new pg.Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:root@localhost:5432/postgres'
});

// Configurar adapter com o pool
const adapter = new PrismaPg(pool);
const prisma = new PrismaClient({ adapter });

async function testPrismaFinal() {
  try {
    console.log('🔄 Testando conexão Prisma com schema john_deere...');
    
    // Primeiro, definir o search_path explicitamente para todas as conexões
    await pool.query('SET search_path TO john_deere');
    
    // Testar conexão listando usuários
    const userCount = await prisma.users.count();
    console.log(`✅ Conexão estabelecida! Total de usuários: ${userCount}`);
    
    // Listar primeiros usuários
    const users = await prisma.users.findMany({
      take: 5,
      select: {
        id: true,
        email: true,
        name: true,
        matricula: true,
        role: true,
        is_active: true,
        created_at: true
      }
    });
    
    console.log('\n👥 Usuários encontrados:');
    users.forEach(user => {
      console.log(`  - ${user.name} (${user.email}) - Matrícula: ${user.matricula} - Role: ${user.role} - ${user.is_active ? 'Ativo' : 'Inativo'}`);
    });
    
    // Listar times/equipes
    const teams = await prisma.teams.findMany({
      take: 5,
      select: {
        id: true,
        name: true,
        description: true,
        is_active: true
      }
    });
    
    console.log('\n👥 Times/Equipes encontrados:');
    teams.forEach(team => {
      console.log(`  - ${team.name} ${team.description ? `(${team.description})` : ''} - ${team.is_active ? 'Ativo' : 'Inativo'}`);
    });
    
    // Verificar application_users
    const appUsers = await prisma.application_users.findMany({
      take: 3,
      select: {
        id: true,
        username: true,
        matricula: true,
        is_active: true,
        created_at: true
      }
    });
    
    console.log('\n📱 Application Users encontrados:');
    appUsers.forEach(user => {
      console.log(`  - ${user.username} - Matrícula: ${user.matricula} - ${user.is_active ? 'Ativo' : 'Inativo'}`);
    });
    
    // Verificar logs de acesso
    const accessLogs = await prisma.access_logs.count();
    console.log(`\n📊 Total de logs de acesso: ${accessLogs}`);
    
    // Verificar permissões do sistema
    const permissions = await prisma.system_permissions.findMany({
      take: 5,
      select: {
        name: true,
        resource: true,
        action: true
      }
    });
    
    console.log('\n🔐 Permissões do sistema:');
    permissions.forEach(perm => {
      console.log(`  - ${perm.name}: ${perm.resource}.${perm.action}`);
    });
    
  } catch (error) {
    console.error('❌ Erro ao conectar com Prisma:', error.message);
    console.error('Código do erro:', error.code);
  } finally {
    await prisma.$disconnect();
    await pool.end();
  }
}

testPrismaFinal();